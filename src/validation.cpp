		...
		for (auto &block : blocks_list) {
			...
			// with no parent all returned valid:true.
			{
				...
				if (level_null) {
					...
					// Level 0 is the explicit document root, and nothing else may sit there.
					// SEVERAL are permitted in one relation: a list can carry many documents --
					// that is what the filename provenance field is for -- and each root opens
					// the next, so a level-0 `document` row is legal wherever it appears. What
					// stays illegal at 0 is every OTHER row: `metadata` at 0 is a top-level
					// blob, not a document. Teague's ruling, 2026-09-16; the 1-based top was
					// chosen to leave 0 free for exactly this.
					const bool is_document_root =
					    lvl == 0 && kind == BlockTypes::KIND_BLOCK && element_type == BlockTypes::TYPE_DOCUMENT;
					if (lvl < 0 || (lvl == 0 && !is_document_root)) {
						child_list_t<Value> error_values;
						error_values.push_back(make_pair("element_order", Value(element_order)));
						error_values.push_back(make_pair("field", Value("level")));
						error_values.push_back(
						    make_pair("message", Value("level " + std::to_string(lvl) +
						                               " is below 1; top level is 1, and level 0 is only the document "
						                               "root: a kind='block' element_type='document' element")));
						errors.push_back(Value::STRUCT(std::move(error_values)));
