...
// Escape a string for embedding in a JSON string literal. Unlike the previous
// hand-rolled versions, this escapes *all* control characters below 0x20
// (as \u00XX), not just \n, \r and \t, so the emitted JSON is always valid.
inline std::string PandocJsonEscape(const std::string &s) {
	std::string result;
...
// Find the position one past the bracket matching json[start] (which must be
// the opening bracket). Skips over JSON string literals, so brackets that
// appear inside document text no longer unbalance the scan (issue #11:
// quote-blind bracket matching truncated extraction).
inline size_t PandocFindMatchingBracket(const std::string &json, size_t start, char open, char close) {
	int count = 1;
	size_t pos = start + 1;
