...
public:
	...
	// single text child. One rule for inline and block containers alike -- a string
	// argument IS a single text child, so it lands in `content`; a list of children
	// leaves content empty and puts them at level+1.
...
private:
	...
	// duck_block_heading(level INTEGER, content) -> LIST(duck_block)
	...
	// duck_block_blockquote(content) -> LIST(duck_block) (level defaults to 1)
	// duck_block_blockquote(level INTEGER, content) -> LIST(duck_block)
	...
	// duck_block_list_item(content) -> LIST(duck_block)
	// duck_block_list_item(ordered BOOLEAN, content) -> LIST(duck_block)
