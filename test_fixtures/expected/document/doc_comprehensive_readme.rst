...
A **comprehensive** template for building `DuckDB <https://duckdb.org>`__ extensions with *modern C++ practices* and full CI/CD support.

   **Important:** This template requires DuckDB v1.0+ and a C++17 compatible compiler. See `Requirements <#requirements>`__ for details.
...
We welcome contributions! Please read our `Contributing Guide <CONTRIBUTING.md>`__ before submitting PRs.

   **Note for contributors:** All PRs must include tests and pass CI checks. Run ``make format`` before committing.

--------------

`Report Issues <https://github.com/user/extension/issues>`__ | `Discord <https://discord.gg/duckdb>`__ | `DuckDB Docs <https://duckdb.org/docs>`__
