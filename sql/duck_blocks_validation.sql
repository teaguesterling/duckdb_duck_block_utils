...
CREATE OR REPLACE MACRO db_block_type_is_known(block_type) AS (
    block_type IN (
...
-- Check if a duck_block has valid structure
CREATE OR REPLACE MACRO db_block_is_valid(block) AS (
    block IS NOT NULL
...
CREATE OR REPLACE MACRO db_block_encoding_is_valid(block) AS (
    block.encoding IS NULL
    OR block.encoding IN ('text', 'markdown', 'duck_inlines', 'yaml', 'json', 'html', 'latex')
);
...
CREATE OR REPLACE MACRO db_inline_type_is_known(inline_type) AS (
    db_inline_is_leaf_type(inline_type)
    OR db_inline_is_whitespace_type(inline_type)
    OR db_inline_is_container_type(inline_type)
    OR inline_type LIKE 'x-%'  -- Custom extension types
);

-- Check if a duck_inline has valid basic structure
CREATE OR REPLACE MACRO db_inline_is_valid(inline) AS (
    inline IS NOT NULL
...
CREATE OR REPLACE MACRO db_inline_whitespace_is_valid(inline) AS (
    NOT db_inline_is_whitespace_type(inline.inline_type)
...
--
-- Parameters:
--   inline: the duck_inline struct
--   has_nested_children: boolean indicating if there are children at level+1
CREATE OR REPLACE MACRO db_inline_content_is_canonical(inline, has_nested_children) AS (
    CASE
        -- Leaf types: always use content field
        WHEN db_inline_is_leaf_type(inline.inline_type) THEN
            true
        -- Whitespace: content must be empty
        WHEN db_inline_is_whitespace_type(inline.inline_type) THEN
            coalesce(inline.content, '') = ''
        -- Container types: content XOR nested children (not both, not neither)
        WHEN db_inline_is_container_type(inline.inline_type) THEN
...
CREATE OR REPLACE MACRO db_blocks_all_valid(blocks) AS (
    list_bool_and([db_block_is_valid(b) FOR b IN blocks])
);

-- Check if all inlines in a list are valid
CREATE OR REPLACE MACRO db_inlines_all_valid(inlines) AS (
    list_bool_and([db_inline_is_valid(i) FOR i IN inlines])
);
...
CREATE OR REPLACE MACRO db_inlines_levels_valid(inlines) AS (
...
CREATE OR REPLACE MACRO db_block_validation_error(block) AS (
    CASE
...
CREATE OR REPLACE MACRO db_inline_validation_error(inline) AS (
    CASE
        ...
        WHEN db_inline_is_whitespace_type(inline.inline_type)
